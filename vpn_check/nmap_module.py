...
class nmapModule:
    ...
    def __init__(self, target=None, nmap_install=False):
        ...
        """
        self.trace_countries = None
        self.trace_coordinates = None
        self.trace_ip = None
        self.trace_address = None
        self.traceroute_data = None
        ...
        self.sqlite = sqlite3.connect(self.fw.direction_path / 'Data_storage' / 'vpn.db')
        self.cur = self.sqlite.cursor()
        self.ch_sql = ClickHouse()
        self.score = 0
    ...
    def db_search_IP(self, table_list=None):
        ...
        for i in table_list:
            ...
            if len(cur.fetchall()) > 0:
                ...
                self.fw.write_in_file(path=self.tmp_result / f'{self.target}.txt', 
                                      message='IP FOUND ON VPN DATABASE. PROBABILITY OF VPN IS VERY HIGH')
    ...
    def retrieving_geo(self):
        ...
        """
        geolocation = self.command_exec(f'cd C:\\Program Files (x86)\\Nmap && '
                                        f'nmap --script ip-geolocation-geoplugin {self.target}')
        if geolocation.find(', try -Pn'):
            geolocation = self.command_exec(f'cd C:\\Program Files (x86)\\Nmap && '
                                            f'nmap -Pn --script ip-geolocation-geoplugin {self.target}')
        arr_values = []
        for i in geolocation.split('\n'):
    ...
    def whois_ip_nmap(self, country='RU'):
        ...
        """
        owner = self.command_exec(f'cd C:\\Program Files (x86)\\Nmap && '
                                       f'nmap {self.target} --script whois-ip')
        if owner.find(', try -Pn'):
            owner = self.command_exec(f'cd C:\\Program Files (x86)\\Nmap && '
    ...
    def traceroute_with_geo(self):
        """
        Traceroute's information about target IP address
        :return: info path to IP address
        """
        trace = self.command_exec(f'cd C:\\Program Files (x86)\\Nmap && '
                                       f'nmap --traceroute --script traceroute-geolocation {self.target}')
        if trace.find(', try -Pn'):
            trace = self.command_exec(f'cd C:\\Program Files (x86)\\Nmap && '
        ...
        arr_values.insert(0, '\n')
        self.fw.write_in_file(self.tmp_result, '\n'.join(arr_values))
        self.traceroute_data = arr_values.copy()
        return arr_values

    def traceroute_parse(self, run=True):
        """
        Func for retrieving all interesting data from traceroute
        response in nmap module
        :param run: If True - run traceroute script on body of
        traceroute parse when traceroute data didn't found in
        nmapModule class. Else - raise error.
        :return: None
        """
        if self.traceroute_data is None and run:
            self.traceroute_with_geo()
            self.traceroute_parse()
        elif self.traceroute_data is None and not run:
            raise ValueError(r'Cannot find traceroute data! Did traceroute was running in current session?')
        elif self.traceroute_data is not None:
            for i in self.traceroute_data:
                tmp_arr = i.split(' ')
                while not self.err:
                    try:
                        tmp_arr.remove('')
                        ...
                        self.err = True
                self.trace_address = tmp_arr[2]
                self.trace_ip = tmp_arr[3].replace('(', '').replace(')', '')
                self.trace_coordinates = tmp_arr[4]
                self.trace_countries = tmp_arr[5]
